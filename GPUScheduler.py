...
def read_option():
    ...
    # basic settings
    parser.add_argument('--m', help='the least memory needed for each command', type=int, default=100)
    parser.add_argument('--p', help='the period of confirming the GPU is being adequately free (measured by seconds), default as 30s', type=int, default=30)
    parser.add_argument('--i', help='the interval between every two checking frames, which should be smaller than `p`', type=int, default=5)
    parser.add_argument('--d', help='the working dictionary for the command', type=str, default='.')
    parser.add_argument('--b', help='the bin dictionary for the file of the commands', type=str, default='')
    try: option = vars(parser.parse_args())
    except IOError as msg: parser.error(str(msg))
    # The maximum of the allocated memory(MB) of a gpu to a command
    global LEAST_FREE_MEMORY, AVAILABLE_PERIOD, CHECK_INTERVAL, COMMAND_WORKING_DIR, BIN_DIR
    LEAST_FREE_MEMORY = option['m']
    AVAILABLE_PERIOD = option['p']
    CHECK_INTERVAL = min(option['i'], AVAILABLE_PERIOD)
    COMMAND_WORKING_DIR = option['d']
    BIN_DIR = option['b']
    if BIN_DIR[-1]!='/': BIN_DIR+='/'
    return option['cmd_file']

def read_cmds_from_txt(filepath):
...
class DeviceManager:
    ...
    def run_cmd(self):
        ...
        if cmd!="":
            ...
            self.ready_start = time.time()
            p = subprocess.Popen(cmd, shell=True, cwd=COMMAND_WORKING_DIR, stdout=subprocess.PIPE, stdin=subprocess.PIPE)
            self.pool.add(p)
...
filepath = read_option()
cmds = read_cmds_from_txt(filepath)
num_cmds = len(cmds)
